...
export const useMusicStore = defineStore("music", {
  ...
  getters: {
    ...
    canPlay: (state) =>
      (state.appState === "idle" || state.appState === "paused_playback") &&
      state.recordedNotes.length > 0,
    canPause: (state) => state.appState === "playing",
    canStopPlayback: (state) =>
      state.appState === "playing" || state.appState === "paused_playback",
  },
  actions: {
    // --- 永続化関連 ---
    loadAllScores() {
    ...
    saveScoresToLocalStorage() {
      try {
        localStorage.setItem(
          LOCAL_STORAGE_KEY,
          JSON.stringify(this.savedScores)
        );
    ...
    addManualNoteEvent(event: Omit<NoteEvent, "id">) {
      ...
      const idx = this.recordedNotes.findIndex(
    ...
    releaseNote(midiNote: number) {
      if (this.isRecording) {
        const now = performance.now();
        const i = this.recordedNotes.findIndex(
          (n) => n.midiNote === midiNote && n.duration === 0
        );
        if (i !== -1) {
    ...
    stopPlayback() {
      ...
      this.recordedNotes.forEach((note) => {
        if (note.midiNote > -1) stopNote(note.midiNote);
      });
      this.appState = "idle";
      console.log("再生停止");
    },
